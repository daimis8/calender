...
let currentMonthShowing: number;
let currentYearShowing: number;
...
if (smallCalenderLeftArrow && smallCalenderRightArrow) {
...
if(headerLeftArrow && headerRightArrow) {
...
function getCalenderInfo(year: number, month: number, day?: number) : CalenderInfo {
...
function setAsideCalender(year : number, month: number, monthName : string, daysInMonth : number, firstDayOfWeek : number, currentDay?: number | null) {
    const asideCalenderHeader = document.querySelector('.aside-calender-month-year');
    if (asideCalenderHeader) {
    ...
    if (asideCalenderDays) {
        // Don't do anything
    } else {
        console.error("No asideCalenderDays")
        return;
    }
...
function createOverlayGrid() {
    ...
    if(bigCalender) {
        ...
        console.error("There is no big calender!")
        return;
    }
...
function getDateFromDayColumn(dayCol: number) {
...
function openModalWithDateTime(date: Date, hour: number, minutes: number) {
    if (modal) {
        modal.style.display = 'flex';
    } else {
        return;
    ...
    const dateSelect = document.getElementById('eventDate') as HTMLSelectElement;
    if (dateSelect) {
        dateSelect.value = dateString;
    } else {
        console.error("No dataSelect");
        return;
    }
    
    
    const startTimeSelect = document.getElementById('eventStartTime') as HTMLSelectElement;
    ...
    const endTimeSelect = document.getElementById('eventEndTime') as HTMLSelectElement;
    if(endTimeSelect) {
        endTimeSelect.value = endTimeString;
    } else {
        console.error("There is no endTimeSelect");
    }
...
function updateHeaderDate() {
    ...
    if (headerDate) {
        ...
        console.error("There is no headerDate");
        return;
    }
...
function initializeDateTimeSelects(clickedDate?: string) {
    const dateSelect = document.getElementById('eventDate') as HTMLSelectElement;
    const startTimeSelect = document.getElementById('eventStartTime') as HTMLSelectElement;
    const endTimeSelect = document.getElementById('eventEndTime') as HTMLSelectElement;
...
function populateAllTimeOptions(timeSelect: HTMLSelectElement) {
...
function validateForm() {
    ...
    const endTime = document.getElementById('eventEndTime') as HTMLSelectElement;
...
function saveEvent() {
    ...
    if(!formTitle || !eventDate || !startTime || !endTime || !description) {
        return;
    }
...
function getWeekDateRange(referenceDate: Date) {
    const startOfWeek = getStartOfWeek(referenceDate);
...
function getDayColumnFromDate(date: Date) {
    const startOfWeek = getStartOfWeek(currentWeekReference);
    ...
    const daysDiff = Math.floor((normalizedEventDate.getTime() - normalizedStartOfWeek.getTime()) / (1000 * 60 * 60 * 24));
    return daysDiff;
}

function timeStringToPosition(timeString: string) {
...
function displayEventsOnCalendar() {
    ...
    weekEvents.forEach((event: EventInfo) => {
        const eventDate = new Date(event.date);
